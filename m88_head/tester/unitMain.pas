...
begin
  if btn_enable_control.Caption = 'Подключиться' then
  begin
    try
      if com.Port = '' then com.Port := 'COM145';
      com.Open
    except
      on e: Exception do
      begin
    ...
    com_receive := '';
    // интерфейс
    btn_enable_control.Caption := 'Отключиться';
    btn_on.Enabled := true;
    ...
    com_receive := '';
    // интерфейс
    btn_enable_control.Caption := 'Подключиться';
    btn_on.Enabled := false;
    btn_off.Enabled := false;
    btn_com_refresh.Enabled := true;
    cb_com_baudrate.Enabled := true;
