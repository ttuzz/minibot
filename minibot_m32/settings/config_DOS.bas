...
' Количество одновременно открытых файлов (каждый файл требует 524 байта SRAM)
Const Cfilehandles = 1                                      ' [по_умолчанию = 2]

' Количество FAT-Buffer в SRAM:
' 0 = FAT- and DIR-Buffer занимают один буфер в SRAM размером 561 байт
' 1 = FAT- and DIR-Buffer размещаются в двух разделенных SRAM буферах общим размером 1078 байт
' Parameter 1 inc reased speed of file-handling
Const Csepfathandle = 0                                     ' [по_умолчанию = 1]

' Handling of pending FAT and Directory information of open files
' 0 = FAT and Directory Information обновляются каждый раз, когда сектор файла обновляется
' 1 = FAT and Directory Information обновляются только при вызове FLUSH и SAVE команд
' Значение 1 резко поднимает скорость записи
Const Cfatdirsaveatend = 1                                  ' [по_умолчанию = 1]


' Окружать строки в выводе команды WRITE двойными кавычками
' 0 = Не окружать
' 1 = Окружать (напр, "Text")
Const Ctextquotationmarks = 1                               ' [по_умолчанию = 1]


' Записывать second FAT. ОС Windows позволяет его не обновлять.
' Если же требуется заполнение second FAT, то можно на PC выполнить команду chdisk /f, 
' она перезаписывает second FAT first FAT'ом
' Значение 0 существенно ускоряет процесс записи файла
' 0 = Second FAT не обновляется
' 1 = Second FAT обновляется если существует
Const Cfatsecondupdate = 0                                  ' [по_умолчанию = 1]

' Символ, разделяющий ASCII значения в выводе команды WRITE (INPUT)
' По умолчанию используется запятая (,), но её можно заменить на любой другой символ, например
' TAB (ASCII-код 9). Значение - это ASCII-код символа
' 44 = , [по_умолчанию]
' 9 = TAB                                                  
Const Cvariableseparator = 44                               ' [по_умолчанию = 44]
