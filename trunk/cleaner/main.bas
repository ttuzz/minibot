...
Declare Sub обработка_бамперов()
   ...
   Dim буфер As Byte
   Dim сработал_бампер As Boolean : Reset сработал_бампер
   Dim бампер_активен As Boolean : Reset бампер_активен
   Dim активировать_бампер As Boolean : Reset активировать_бампер
...
Text = "off" : Call Printf(text) : Reset бампер_активен : Reset активировать_бампер
Do
...
получить_символ:
   ...
   Else                                                     'in_string содержит принятую строку
      ...
      If In_string = "boot" Then Goto &H3C00
...
Sub обработка_бамперов()
   Local макс_расстояние As Word
   макс_расстояние = Max(расстояния(1))
   If макс_расстояние > 90 Then
      If состояние_робота = управление_пультом And сработал_бампер = 0 Then
      Disable Int1
      ...
      End If
   Else
      Reset сработал_бампер
   End If
   If активировать_бампер = 1 And бампер_активен = 0 Then
      Text = "on" : Call Printf(text)
      Set бампер_активен
      Reset активировать_бампер
   End If
End Sub
   ...
   ' команды движения
      ...
      Case Btn_left:
         ...
         Case 0 To 3 :
            скорость_левый = 200 : скорость_правый = 200
         Case 3 To 5 :
            скорость_левый = 230 : скорость_правый = 230
         Case 6 To 255 :
